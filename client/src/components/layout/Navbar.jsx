...
import { Menu, LogIn, LogOut, Settings, User, ChevronDown, Map, Globe, PlusCircle } from "lucide-react";
...
function ProfileMenu() {
  ...
  if (loading) {
    return (
      <div className="flex items-center justify-center w-10 h-10 text-lg font-semibold text-white bg-teal-500/70 rounded-full animate-pulse">
  ...
  return (
    <div className="relative" ref={dropdownRef}>
      <button
        ...
        className="flex items-center gap-2 px-3 py-2 rounded-full bg-gradient-to-r from-teal-600 to-blue-600 text-white focus:outline-none focus:ring-2 focus:ring-offset-2 focus:ring-teal-300 transition-all duration-300 hover:shadow-md"
        id="user-menu"
        aria-haspopup="true"
      ...
      {isOpen && (
        <div
          className="origin-top-right absolute right-0 mt-2 w-64 rounded-xl shadow-xl py-2 bg-white ring-1 ring-black ring-opacity-5 z-50 transition-all duration-200 ease-out transform"
          role="menu"
          ...
          {userData ? (
            <>
              ...
              <Link
                ...
                onClick={() => setIsOpen(false)}
              >
                <Map className="mr-3 h-4 w-4 text-blue-600" /> 
...
function MobileMenu({ isOpen, setIsOpen, isAuthenticated }) {
  ...
  return (
    <div className="sm:hidden bg-gradient-to-r from-teal-600 to-blue-600 text-white py-2 px-2 rounded-b-lg shadow-lg">
      <div className="pt-2 pb-3 space-y-1">
        <NavItem to="/" label="Home" icon={<Globe className="h-4 w-4 mr-2" />} onClick={() => setIsOpen(false)} />
        {isAuthenticated && (
          <>
            <NavItem 
              to="/my-itineraries" 
...
export default function Navbar() {
  ...
  return (
    <nav className={`fixed w-full top-0 z-50 transition-all duration-300 ${
      scrolled 
        ? "bg-gradient-to-r from-teal-600 to-blue-600 shadow-lg" 
        : "bg-gradient-to-r from-teal-500/90 to-blue-500/90 backdrop-blur-sm"
    } rounded-b-lg`}>
        ...
        <div className="flex justify-between h-16">
          <div className="flex items-center">
            ...
            <div className="hidden sm:ml-6 sm:flex sm:space-x-8">
              <NavItem to="/" label="Home" icon={<Globe className="h-4 w-4 mr-2" />} />
              {isAuthenticated && (
                <>
                  <NavItem to="/my-itineraries" label="My Itineraries" icon={<Map className="h-4 w-4 mr-2" />} />
